...
###### 构建带有 SGD 训练的线性分类器
def svm(X, x_train, y_train, x_test, y_test, kernel, max_iter):
    '''
    构建带有 SGD 训练的线性分类器对训练集和测试集进行预测，并对整个数据集进行预测
    :param X: 整个数据集的自变量
    :param x_train:
    :param y_train:
    :param x_test:
    ...
    :return:
    '''
    clf = SVC(gamma='auto', probability=True, kernel=kernel, max_iter=max_iter)
...
###### 只需要调用这一个函数即可（主函数）
def svcModel(filepath, x_cols, y_col, kernel, max_iter):
    if(type(x_cols)==str):
    ...
    # 训练模型
    y_train, y_test, y_test_pred, y_train_pred, y_pred, y_pred_prob = svm(X, X_train, y_train, X_test, y_test, kernel=kernel, max_iter=max_iter)
    # 预测的数字逆转码
...
###### 画图的函数
def paintSVM(filepath, x_cols, y_col):
    ...
    for i in kernels:
        for j in range(1, 11):
            train_score, test_score, df, filename = svcModel(filepath, x_cols, y_col, i, j)
            if(i=='linear'):
                linear_test.append(test_score)
                linear_train.append(train_score)
            elif(i=='poly'):
                poly_train.append(train_score)
                poly_test.append(test_score)
            elif(i=='rbf'):
                rbf_train.append(train_score)
                rbf_test.append(test_score)
            elif(i=='sigmoid'):
                sigmoid_train.append(train_score)
                sigmoid_test.append(test_score)
            else:
                precomputed_test.append(test_score)
                precomputed_train.append(train_score)
    return dataList, linear_train, linear_test, poly_train, poly_test, rbf_train, rbf_test, sigmoid_train, sigmoid_test, precomputed_train, precomputed_test
...
if __name__ == '__main__':
    ...
    y_col = '用户类型'
    # train_score, test_score, df, savepath = svcModel(filepath, x_cols, y_col)
    # print(train_score, test_score, df, savepath)
    print(paintSVM(filepath, x_cols, y_col))
