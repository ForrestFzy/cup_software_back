...
def build_firstPart_model(input_var,keep_prob_=0.5):
        ...
        # Dropout
        network = tf.nn.dropout(network, keep_prob_)     # noise_shape代表随机生成的形状
...
def build_network(hparams,char2numY,inputs,dec_inputs,keep_prob_=0.5,):
    ...
    with tf.variable_scope("encoding") as encoding_scope:
        if not hparams.bidirectional:
            ...
            encoder_outputs, encoder_state = tf.compat.v1.nn.dynamic_rnn(encoder_cell, inputs=data_input_embed, dtype=tf.float32)
...
def build_whole_model(hparams,char2numY,inputs, targets,dec_inputs, keep_prob_):
    ...
    # print(logits.summary())  # AttributeError: 'Tensor' object has no attribute 'summary'
...
def main(args=None):
    ...
    tf.app.flags.DEFINE_string('output_dir', 'output_2013/outputs_eeg_pz_oz',
