...
###### 构建带有 SGD 训练的线性分类器
def CNB(X, x_train, y_train, x_test, y_test):
    '''
...
def gaussModel(filepath, x_cols, y_col):
    ...
    # 训练模型
    y_train, y_test, y_test_pred, y_train_pred, y_pred, y_pred_prob = CNB(X, X_train, y_train, X_test, y_test)
    # 预测的数字逆转码
    ...
    del df[df.columns[0]]
    savepath = os.path.join('../outputFiles3', filename)
